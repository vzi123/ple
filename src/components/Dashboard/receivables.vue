<template>
  <div class="card-body p-xl-40">
    <h6 class="card-title fw-semiBold fs-18 mb-30">RECIEVABLES</h6>
      ...
      <table class="table text-nowrap align-middle mb-0">
        ...
        <tbody v-for="(purchaseItem, index) in allProducts" :key="index">
                  <tr>
                    ...
                    <td class="shadow-none lh-1 fs-14 fw-normal text-paragraph">{{ purchaseItem.status }}</td>
                     <td class="shadow-none lh-1 fs-14 fw-normal text-paragraph">{{ purchaseItem.comments }}</td>
                      <td class="shadow-none lh-1 fs-14 fw-normal text-paragraph">{{ purchaseItem.salesOrderId }}</td>
                  </tr>
  ...
  setup() {
    ...
    const fetchProducts = async () => {
      try {
        ...
        const response = await axios.get(`${BASE_URL}/freezy/dashboard/receivables`);
