...
const currentSelection = computed(() => menuVisible.value && state.index != -1 && state.index < filteredItems.value.length ? filteredItems.value[state.index] : undefined);
...
const onBlur = () => {
    ...
    if (!state.onBlurIgnoreBuffer) {
        if (props.allowNew && (buffer.value || '').length > 0) {
            const match = (Array.isArray(props.items) ? props.items : []).filter((item) => props.itemProjection(item).toLowerCase() == buffer.value.toLowerCase());
...
const filterItems = () => {
    ...
    if (Array.isArray(props.items)) {
        ...
        console.error("`items` prop should be either an array or a function.");
        loadItems([]); // Safeguard in case items is neither an array nor a function
    }
};
</script>
